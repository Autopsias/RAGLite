...
from mcp_server import QueryRequest, execute_query
...
async def validate_question(qa: dict[str, Any], top_k: int = 5) -> dict[str, Any]:
    ...
    match_score = (
        len(matched_keywords) / len(qa["expected_keywords"]) if qa["expected_keywords"] else 0
    )
    ...
    return {
        ...
        "top_chunks": [
            {"score": r.score, "text_preview": r.text[:200], "chunk_index": r.chunk_index}
