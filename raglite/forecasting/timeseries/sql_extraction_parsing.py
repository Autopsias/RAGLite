...
Story 9.8: Simplified to use pre-classified data from database instead of
runtime period classification.
"""
...
class ParsedTimeSeriesData:
    ...
    Story 9.8: Removed classification_report field (classification now done at ingestion).
    """
...
def parse_sql_rows_with_units(
    ...
    Story 9.8: Simplified - classification is now done at ingestion time and
    filtered via SQL WHERE clauses, so no runtime classification needed.
    ...
    for row in rows:
        ...
        # Parse period for YTD data (extract Mon-YY from "YTD Mon-YY")
        parse_period = period_str
        if period_str and period_str.startswith("YTD"):
            # Extract normalized period from "YTD Mon-YY" -> "Mon-YY"
            match = re.search(r"([A-Za-z]{3}-[0-9]{2,4})", period_str)
            if match:
                parse_period = match.group(1)
