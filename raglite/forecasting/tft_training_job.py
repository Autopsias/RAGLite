...
def _get_active_data_sources(session: Any) -> list[Any]:
    """Get all active (non-deleted) external data sources.

    Args:
        session: Database session

    Returns:
        List of ExternalDataSourceORM objects
    """
...
def _collect_training_data(
    ...
    Args:
        storage: External data storage instance
        sources: List of data source ORM objects
...
async def _train_and_save_tft_model(
    df: pd.DataFrame,
) -> tuple[str, dict[str, float], int, int]:
    """Train TFT model and save checkpoint.

    Args:
        df: Training data DataFrame

    Returns:
        Tuple of (checkpoint_path, metrics, training_samples, validation_samples)
    """
    # Prepare datasets
    ...
    return checkpoint_path, metrics, int(len(training_dataset)), int(len(validation_dataset))  # type: ignore[return-value]
...
async def run_weekly_tft_training() -> None:
    ...
    try:
        ...
        sources = _get_active_data_sources(session)
        ...
        all_data = _collect_training_data(
            storage=storage,
            sources=sources,
            start_date=start_date,
            end_date=end_date,
        ...
        (
            ...
            training_samples,
            validation_samples,
        ) = await _train_and_save_tft_model(df=df)
