...
def _build_recommendation_prompt(insight: Insight, category: RecommendationCategory) -> str:
    """Build the LLM prompt for recommendation synthesis.
...
def _parse_llm_response(content: str) -> tuple[str, str, str, list[str]]:
    """Parse LLM response into recommendation components.

    Args:
        content: Raw LLM response string

    Returns:
        Tuple of (title, description, rationale, actions)
    """
    title = ""
    description = ""
    rationale = ""
...
def _apply_fallbacks(
    ...
    Args:
        ...
        actions: Parsed actions (may be empty)
        insight: Input insight for fallback content
        category: Recommendation category

    Returns:
        Tuple with all components populated
    """
    if not title:
        title = f"{category.value.replace('_', ' ').title()} Recommendation"
    if not description:
...
async def synthesize_recommendation(
    ...
    prompt = _build_recommendation_prompt(insight, category)
    ...
    try:
        ...
        return _get_fallback_recommendation(insight, category)
