...
def _prepare_chunk_record(chunk: Chunk) -> tuple:
    """Prepare a single chunk for PostgreSQL insertion.

    Args:
        chunk: Chunk object to convert to database record

    Returns:
        Tuple of values for financial_chunks table insert
    """
    return (
...
def _execute_batch_insert(
    cursor: Any, batch_records: list[tuple], batch_num: int, total_batches: int
) -> None:
    """Execute a single batch insert into financial_chunks table.

    Args:
        cursor: PostgreSQL cursor for executing queries
        batch_records: List of record tuples to insert
...
def _cleanup_connection(cursor: Any, conn: Any) -> None:
    """Clean up database connection after error.

    Args:
        cursor: PostgreSQL cursor to close
        conn: PostgreSQL connection to rollback
    """
    try:
        if cursor:
            cursor.close()
...
async def store_metadata_in_postgresql(
    ...
    try:
        ...
        records = [_prepare_chunk_record(chunk) for chunk in chunks_to_store]
        ...
        for i in range(0, len(records), batch_size):
            ...
            _execute_batch_insert(cursor, batch_records, batch_num, total_batches)
        ...
        _cleanup_connection(cursor if "cursor" in locals() else None, conn)
