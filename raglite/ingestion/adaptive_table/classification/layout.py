...
def _classify_headers(
    table_cells: list,
) -> tuple[dict[int, dict[HeaderType, int]], dict[HeaderType, int], dict[int, list]]:
    """Classify table headers by type and position.

    Args:
        table_cells: List of table cells from Docling

    Returns:
        Tuple of (col_header_types, row_header_type_counts, col_header_by_row)
    """
    ...
    return col_header_types, row_header_type_counts, col_header_by_row


def _detect_multi_header_metric_entity(
    col_header_by_row: dict[int, list],
    col_header_types: dict[int, dict[HeaderType, int]],
    metadata: dict[str, Any],
) -> tuple[TableLayout, dict[str, Any]] | None:
    """Detect strict multi-header pattern: Metric (Row 0) + Entity (Row 1).

    Args:
        col_header_by_row: Column headers grouped by row
        col_header_types: Header type counts by row
        metadata: Base metadata dict to update

    Returns:
        (TableLayout, metadata) if pattern matched, None otherwise
    """
    if len(col_header_by_row) < 2:
        return None
...
def _detect_transposed_table(
    table_cells: list,
    col_header_types: dict[int, dict[HeaderType, int]],
    is_multi_header: bool,
    metadata: dict[str, Any],
) -> tuple[TableLayout, dict[str, Any]] | None:
    """Detect transposed table pattern: metrics in first column, entities in headers.

    Args:
        table_cells: List of table cells
        col_header_types: Header type counts by row
        is_multi_header: Whether table has multiple header rows
        metadata: Base metadata dict to update
    ...
    if not first_col_cells or len(first_col_cells) < 3:  # At least 3 data rows
        return None
    ...
    col_header_entity_count = sum(
        count for htype, count in col_header_types.get(0, {}).items() if htype == HeaderType.ENTITY
    )
...
def _detect_relaxed_multi_header(
    col_header_by_row: dict[int, list],
    row_header_type_counts: dict[HeaderType, int],
    metadata: dict[str, Any],
) -> tuple[TableLayout, dict[str, Any]] | None:
    """Detect relaxed multi-header pattern: 2+ header rows with metric row headers.

    Args:
        col_header_by_row: Column headers grouped by row
        row_header_type_counts: Row header type counts
        metadata: Base metadata dict to update
...
def _detect_single_header_patterns(
    col_header_types: dict[int, dict[HeaderType, int]],
    row_header_type_counts: dict[HeaderType, int],
    metadata: dict[str, Any],
) -> tuple[TableLayout, dict[str, Any]] | None:
    """Detect single-header layout patterns (temporal/entity/metric).
...
def detect_table_layout(
    ...
    """
    # Classify headers
    col_header_types, row_header_type_counts, col_header_by_row = _classify_headers(table_cells)
    ...
    if is_multi_header:
        result = _detect_multi_header_metric_entity(col_header_by_row, col_header_types, metadata)
        if result:
            return result
    ...
    result = _detect_transposed_table(table_cells, col_header_types, is_multi_header, metadata)
    if result:
        return result
    ...
    if is_multi_header:
        result = _detect_relaxed_multi_header(col_header_by_row, row_header_type_counts, metadata)
        if result:
            return result

    # Pattern 2: Single column header row
    if not is_multi_header and col_header_by_row:
        result = _detect_single_header_patterns(col_header_types, row_header_type_counts, metadata)
        if result:
            return result
