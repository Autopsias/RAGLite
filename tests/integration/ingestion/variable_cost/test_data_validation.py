"""Integration tests for Variable Cost data validation and entity parameters (Story 6.15).

Tests AC2 (data sufficiency), AC4 (MAPE prerequisites), AC5 (no regression), and entity parameter integration.
"""
...
class TestSufficientDataPoints:
    """Test data point sufficiency for forecasting (AC2).

    Story 6.15: Entity-filtered extraction must return sufficient
    data points for reliable forecasting (minimum 6 monthly periods).
    """

    @pytest.mark.asyncio
    async def test_ac2_minimum_six_data_points(self) -> None:
        """AC2: Extraction returns at least 6 data points for forecasting.

        Given: Variable Cost extraction with entity='portugal' filter
        ...
        if data is None:
            pytest.skip(
                "No Variable Cost data available (extraction failed). Run with TEST_USE_FULL_PDF=true."
            )
    ...
    @pytest.mark.asyncio
    async def test_ac2_data_points_have_valid_dates(self) -> None:
        """AC2: All data points have valid datetime values.

        Given: Variable Cost data extracted with entity='portugal' filter
        When: Checking date fields
        Then: All data points have valid datetime objects
        """
        ...
        for point in data.points:
            if not isinstance(point.date, datetime):
                invalid_dates.append(f"{point.label}: {type(point.date)}")

        assert len(invalid_dates) == 0, (
            f"Found {len(invalid_dates)} points with invalid dates: {invalid_dates}"
        )

    @pytest.mark.asyncio
    async def test_ac2_data_sorted_chronologically(self) -> None:
        """AC2: Data points are sorted chronologically.
        ...
        if data is None or len(data.points) < 2:
            pytest.skip(
                "Insufficient Variable Cost data for sorting check. Run with TEST_USE_FULL_PDF=true."
            )
