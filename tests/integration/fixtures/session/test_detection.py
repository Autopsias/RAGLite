...
Determines which test types are running to enable conditional fixture loading.
This prevents expensive fixture initialization (60-70s for embedding model,
60-150s for PDF ingestion) when only unit tests are running.
"""
...
def _check_pytest_args(request) -> bool | None:
    ...
    Returns:
        True if integration tests found, False if excluded, None if inconclusive
    """
...
def _check_command_line_args() -> bool | None:
...
def _check_session_items(request) -> bool | None:
    """Check session items for integration test ratio.
    ...
    Returns:
        True if integration tests detected, False if not, None if no session items
    """
...
def _check_xdist_worker_node(request) -> bool:
    """Check if current xdist worker node is running integration tests.

    Args:
        request: pytest request fixture

    Returns:
        True if node has integration tests
    """
...
def _check_vs_code_patterns(request) -> bool | None:
    ...
    Returns:
        False if VS Code discovery mode detected, None otherwise
    """
    if not hasattr(request, "config"):
        return None
