...
from tests.integration.helpers.parallel_ingestion_helpers import (
    ...
    print_test_summary,
    reset_postgresql_after_ingestion,
    setup_postgresql_baseline,
    verify_ingestion_result,
    verify_postgresql_storage,
    verify_qdrant_storage,
    verify_storage_systems,
)
...
async def test_parallel_ingestion_three_documents():
    ...
    file_paths = prepare_test_documents()

    # Setup Qdrant
    qdrant = get_qdrant_client()

    # Setup PostgreSQL and clear test data
    postgres_conn, cursor, initial_chunk_count, initial_table_count = setup_postgresql_baseline()

    # Get baseline counts
    initial_qdrant_count, chunk_count, table_count = get_baseline_counts(qdrant, cursor)
    ...
    verify_ingestion_result(result)

    # Verify Qdrant storage
    new_qdrant_chunks = verify_qdrant_storage(qdrant, initial_qdrant_count)

    # Reset PostgreSQL connection after ingestion
    postgres_conn, cursor = reset_postgresql_after_ingestion()

    # Verify PostgreSQL storage
    new_postgres_chunks, new_table_rows = verify_postgresql_storage(
        cursor, initial_chunk_count, initial_table_count
    )

    # Verify both storage systems
    verify_storage_systems(new_qdrant_chunks, new_postgres_chunks)

    cursor.close()

    # Print summary
    print_test_summary(result, new_qdrant_chunks, new_postgres_chunks, new_table_rows)
