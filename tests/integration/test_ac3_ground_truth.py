...
async def _execute_single_ground_truth_query(
    idx: int,
...
def _save_ground_truth_results(
...
async def test_ac1_full_ground_truth_execution() -> AccuracyMetrics:
    ...
    for idx, query_data in enumerate(GROUND_TRUTH_QA, start=1):
        ...
        result, elapsed_ms = await _execute_single_ground_truth_query(idx, query)
