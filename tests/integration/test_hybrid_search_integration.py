...
class TestHybridSearchIntegration:
    ...
    async def _validate_qdrant_collection(self):
    ...
    def _print_test_header(self, collection_info):
    ...
    async def _run_single_query(self, qa, idx, total):
    ...
    async def _run_all_queries(self):
        ...
        for i, qa in enumerate(GROUND_TRUTH_QA, start=1):
            result, latency = await self._run_single_query(qa, i, len(GROUND_TRUTH_QA))
            results.append(result)
            latencies.append(latency)

        return results, latencies

    def _print_accuracy_results(self, metrics):
    ...
    def _validate_retrieval_accuracy(self, metrics):
    ...
    def _validate_attribution_accuracy(self, metrics):
    ...
    def _validate_latency(self, metrics):
    ...
    async def _collect_all_query_results(self):
        """Run all ground truth queries and collect results.

        Returns:
            Tuple of (results list, latencies list, collection_info)
        """
        from scripts.accuracy_utils import calculate_performance_metrics

        # Validate Qdrant collection
        collection_info = await self._validate_qdrant_collection()

        # Print test header
        self._print_test_header(collection_info)

        # Run all queries
        results, latencies = await self._run_all_queries()
    ...
    async def test_hybrid_search_full_ground_truth(self):
        ...
        """
        # Collect all query results and metrics
        results, latencies, metrics, collection_info = await self._collect_all_query_results()

        # Print results
        self._print_accuracy_results(metrics)

        # Validate accuracy and latency
        self._validate_retrieval_accuracy(metrics)
        self._validate_attribution_accuracy(metrics)
        self._validate_latency(metrics)
