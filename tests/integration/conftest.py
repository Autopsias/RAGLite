...
# =============================================================================
# Memory Management Fixture (Phase 5: OOM Prevention)
# =============================================================================


@pytest.fixture(scope="module", autouse=True)
def cleanup_heavy_singletons_after_module():
    """Free memory between test modules to prevent OOM in CI.

    The Fin-E5 embedding model (~2GB) is cached as a singleton.
    When running with pytest-xdist (-n 2+), each worker loads its own copy.
    This fixture clears the cached model after each module completes,
    reducing peak memory usage.

    IMPORTANT: Only triggers garbage collection for embedding model.
    Other singletons (Qdrant client, settings) are lightweight and reusable.
    """
    # Yield first - run tests in this module
    yield

    # After module tests complete, attempt to free heavy singletons
    import gc

    try:
        import raglite.shared.clients as clients

        # Clear embedding model singleton if it exists (~2GB)
        if hasattr(clients, "_embedding_model") and clients._embedding_model is not None:
            print(
                "\n🧹 Clearing embedding model singleton after module (memory optimization)",
                file=sys.stderr,
            )
            del clients._embedding_model
            clients._embedding_model = None
            gc.collect()
    except ImportError:
        pass  # Module not loaded yet - nothing to clean
